...
export const mutations: MutationTree<OnboardState> = {
  ...
  setOnboard: (state, onboardInstance: RLogin) => {
    state.forceUpdateValue++;
    rlogin = onboardInstance;
  },
  setOnboardTheme: (state, theme: "light" | "dark") => {
    state.onboardTheme = theme;
    // if (rlogin) {
    //   rlogin.state.actions.updateTheme(theme);
    // }
  },
...
export const actions: ActionTree<OnboardState, OnboardState> = {
  async onboardInit({ commit, getters }) {
    const rpcUrls: Record<number, string> = {};
    const supportedChains: number[] = [];
    const infoOptions: Record<number, Record<"addressBaseURL", string>> = {};

    Object.entries(getters.ethereumNetworkConfig as ZkEthereumNetworkConfig).forEach(([, network]) => {
      rpcUrls[network.id] = network.rpc_url;
      supportedChains.push(network.id);
      infoOptions[network.id] = {
        addressBaseURL: network.explorer,
      };
    });
  ...
  async reset({ commit, dispatch, state }) {
    ...
    // if (rlogin?.state.get()?.wallets.length) {
    // const [primaryWallet] = rlogin.state.get().wallets;
    if (state.disconnect) {
  ...
  async checkRightNetwork({ getters, commit }) {
    try {
      await new Promise((resolve, reject) => {
        ...
        const walletChainId = wc1Provider?.chainId ?? parseInt(rlogin?.state.get().wallets[0].chains[0].id ?? "", 16);
        ...
        const isWC = !!wc1Provider || (rlogin?.state.get().wallets[0].provider as any)?.connector?.protocol === "wc";
        if (isWC) {
  ...
  async subscribeForOnboardWalletUpdates({ getters, commit, dispatch }) {
    const state = rlogin!.state.select("wallets");
  ...
  async loginWithOnboard({ getters, commit, dispatch }, walletName?: string) {
    ...
    const wallets = rlogin!.state.get().wallets;
    if (!wallets.length) {
